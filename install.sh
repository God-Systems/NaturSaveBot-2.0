...
REPO_TARBALL_URL="${NATURSAVEBOT_TARBALL_URL:-}"
REPO_TARBALL_SHA256="${NATURSAVEBOT_TARBALL_SHA256:-}"
OS_ID=""
OS_VERSION_CODENAME=""
...
bootstrap_from_tarball_if_needed() {
  ...
  [[ -n "${REPO_TARBALL_URL}" && -n "${REPO_TARBALL_SHA256}" ]] || { printf '[natursavebot] ERROR: set NATURSAVEBOT_TARBALL_URL and NATURSAVEBOT_TARBALL_SHA256\n' >&2; exit 1; }
  command -v curl >/dev/null 2>&1 || { printf '[natursavebot] ERROR: curl is required\n' >&2; exit 1; }
  command -v sha256sum >/dev/null 2>&1 || { printf '[natursavebot] ERROR: sha256sum is required\n' >&2; exit 1; }
  ...
  temp_dir="$(mktemp -d)"
  archive="${temp_dir}/natursavebot.tar.gz"
  printf '[natursavebot] Downloading and verifying installer bundle\n'
  curl -fsSL --retry 3 "${REPO_TARBALL_URL}" -o "${archive}"
  printf '%s  %s\n' "${REPO_TARBALL_SHA256}" "${archive}" | sha256sum --check --status || { printf '[natursavebot] ERROR: checksum verification failed\n' >&2; exit 1; }
