...
from src.db.repositories.settings import get_persisted_settings
from src.db.session import init_db
from src.db.session import get_session
from src.logging_config import setup_logging
from src.services.agent import AgentRouter
from src.services.backups import DatabaseBackupService
from src.services.error_tracking import init_error_tracking
...
async def main() -> None:
    ...
    async with get_session() as session:
        persisted_settings = await get_persisted_settings(session)
    ...
    backups = DatabaseBackupService(settings)
    reminder_scheduler.scheduler.add_job(backups.create_backup, "cron", hour=3, id="database_backup", replace_existing=True)
