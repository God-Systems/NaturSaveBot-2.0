        ...
        async with get_session() as session:
            ...
            for reminder in reminders:
                if reminder.remind_at <= now:
                    ...
                    sent_leads = _sent_leads(reminder.lead_sent_json)
                    lead = _next_due_lead(minutes_left, self.settings.reminder_lead_minutes, sent_leads)
                    if lead is not None:
                        try:
                            await self.bot.send_message(
                                self.settings.owner_telegram_id,
                            ...
                            reminder.last_ping_at = now
                            reminder.lead_sent_json = json.dumps(sorted(sent_leads | {lead}))
                        except Exception:
...
def _sent_leads(value: str | None) -> set[int]:
    if not value:
        return set()
    try:
        ...
        return set()
    return {item for item in parsed if isinstance(item, int) and item >= 0} if isinstance(parsed, list) else set()


def _next_due_lead(minutes_left: int, configured: list[int], sent: set[int]) -> int | None:
    available = {lead for lead in configured if lead >= 0 and lead not in sent and minutes_left <= lead}
