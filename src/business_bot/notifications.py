...
async def notify_business_delete(bot: Bot, settings: Settings, message, *, chat_label: str | None = None) -> None:
    ...
    body = _delete_caption(message, title=f"{marker} {title}".strip(), chat_label=chat_label, timezone_name=settings.timezone)
...
async def notify_business_media_saved(bot: Bot, settings: Settings, message, media, *, title: str | None = None) -> None:
    ...
    body = _media_caption(message, title=media_title, media=media, timezone_name=settings.timezone)
...
async def notify_business_edit(
    ...
    *,
    ...
    await _safe_message(
        ...
        f"{_chat_link_from_values(chat_label=chat_label, username=sender_username, chat_id=chat_id)} · {TIME_EMOJI} <code>{_format_date(message_date, settings.timezone)}</code>\n\n"
...
def _delete_caption(message, *, title: str, chat_label: str | None, timezone_name: str) -> str:
    text = getattr(message, "text", None) or getattr(message, "caption", None) or "[медиа без текста]"
    return (
        f"<b>{TRASH_EMOJI} {html_quote(title)}</b>\n\n"
        f"{_chat_link(message, chat_label)} · {TIME_EMOJI} <code>{_format_date(getattr(message, 'date', None), timezone_name)}</code>\n\n"
...
def _media_caption(message, *, title: str, media=None, timezone_name: str, extra: str = "") -> str:
    text = getattr(message, "text", None) or getattr(message, "caption", None) or "[медиа без текста]"
    return (
        f"<b>{html_quote(title)}</b>\n\n"
        f"{_chat_link(message, None)} · {TIME_EMOJI} <code>{_format_date(getattr(message, 'date', None), timezone_name)}</code>\n\n"
...
def _format_date(value, timezone_name: str) -> str:
    if value is None:
        return "-"
    ...
    return dt.astimezone(ZoneInfo(timezone_name)).strftime("%H:%M %d.%m.%y")
