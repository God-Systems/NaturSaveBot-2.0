...
def rights_summary(connection: BusinessConnection | None) -> str:
    ...
    rights_line = ", ".join(html_quote(key) for key in enabled_rights) if enabled_rights else "Telegram API не передал список прав"
    return (
        f"Статус: {'enabled' if connection.is_enabled else 'disabled'}\n"
        f"Business connection: {html_quote(connection.connection_id)}\n"
