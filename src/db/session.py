...
async def init_db(database_url: str | None = None) -> None:
    ...
    async with engine.begin() as conn:
        ...
        await _ensure_sqlite_message_index(conn)
        await conn.execute(
            text(
                """
...
async def _ensure_sqlite_message_index(conn) -> None:
    await conn.execute(
        text(
            """
            ...
            WHERE id NOT IN (
                SELECT kept_id FROM (
                    ...
                    FROM messages
                    GROUP BY source, COALESCE(business_connection_id, ''), chat_id, message_id
                )
            )
            """
        )
    ...
    await conn.execute(
        text(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS uq_messages_source_conn_chat_msg_normalized
            ON messages (source, COALESCE(business_connection_id, ''), chat_id, message_id)
            """
        )
    )
