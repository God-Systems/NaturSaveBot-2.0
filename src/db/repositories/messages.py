...
async def search_messages(session: AsyncSession, query: str, *, limit: int = 20) -> list[Message]:
    ...
    stmt = (
        ...
        .where(
            or_(
                Message.text.ilike(like, escape="\\"),
                Message.sender_name.ilike(like, escape="\\"),
                Message.chat_title.ilike(like, escape="\\"),
            )
        )
...
async def claim_draft_for_send(session: AsyncSession, draft_id: str) -> DraftMessage | None:
    result = await session.execute(
        update(DraftMessage)
        .where(DraftMessage.id == draft_id, DraftMessage.status == "pending")
