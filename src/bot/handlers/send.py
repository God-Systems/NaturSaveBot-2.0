...
from src.db.repositories.messages import claim_draft_for_send, create_draft, get_draft
...
async def send_callback(callback: CallbackQuery, userbot: UserbotManager, state: FSMContext, settings: Settings) -> None:
    ...
    async with get_session() as session:
        ...
        if action == "edit":
            ...
            await callback.answer()
            return
        draft = await claim_draft_for_send(session, draft_id)
        if draft is None:
            await callback.answer("Черновик уже обрабатывается.", show_alert=True)
            return
        await session.commit()
        try:
            if settings.telegram_mode in {"business", "both"}:
                ...
                if client is None:
                    raise RuntimeError("Legacy userbot не подключён.")
            ...
            draft.status = "failed"
            await session.commit()
            await callback.answer("Ошибка отправки; черновик помечен failed, чтобы не отправить текст дважды.", show_alert=True)
            return
        draft.status = "sent"
        await session.commit()
    if isinstance(callback.message, Message):
