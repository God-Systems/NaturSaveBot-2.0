...
async def step_phone(message: Message, state: FSMContext, userbot: UserbotManager) -> None:
    ...
    pending = await userbot.start_pending(message.from_user.id, data["api_id"], data["api_hash"])
    pending.phone = phone
    try:
        ...
        await message.answer("Не удалось отправить код Telegram. Попробуй ещё раз через /login.")
        await state.clear()
        await userbot.cancel_pending(message.from_user.id)
        return
