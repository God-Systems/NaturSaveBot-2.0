...
async def cmd_chat(message: Message) -> None:
    ...
    lines = [
        f"<b>{html_quote(chat.title or str(chat.chat_id))}</b>",
        f"Chat ID: <code>{chat.chat_id}</code>",
        f"Username: <code>{html_quote(('@' + chat.username) if chat.username else '-')}</code>",
        f"Type: <code>{chat.chat_type}</code>",
        "",
        "<b>Последние сообщения:</b>",
    ]
    lines.extend(f"- {html_quote(clip(row.text or row.caption or '[медиа]', 120))}" for row in recent)
