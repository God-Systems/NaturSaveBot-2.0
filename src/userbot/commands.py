...
def attach_user_commands(client: TelegramClient, owner_id: int, settings: Settings) -> None:
    ...
    @client.on(events.NewMessage(outgoing=True, pattern=r"^\.(?:help|помощь)$"))
    async def help_command(event: events.NewMessage.Event) -> None:
        ...
        await event.edit(
            ...
            ".status — current mode\n"
            ".mute / .unmute — hard mute for this chat\n"
            ".info (reply) — user information\n"
            ".type text — send text after typing\n"
            ".repeat N text / .spam_stop — bounded repeat\n"
            ".love — short animation"
        )

    @client.on(events.NewMessage(outgoing=True, pattern=r"^\.(?:status|статус)$"))
    async def status_command(event: events.NewMessage.Event) -> None:
        if not await is_owner(event):
            return
        await event.edit(
            "<b>Userbot status</b>\n"
            f"Save mode: <code>{settings.save_mode_enabled}</code>\n"
            f"Hard mute: <code>{settings.enable_hard_mute}</code>\n"
            f"LLM: <code>{settings.llm_provider}</code>\n"
            f"Active repeats: <code>{len(_repeat_tasks)}</code>"
        )
...
async def _repeat_worker(
    *,
    ...
    try:
        ...
        if _repeat_tasks.get(chat_id) is asyncio.current_task():
